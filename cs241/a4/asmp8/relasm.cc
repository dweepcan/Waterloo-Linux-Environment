...
int main(int argc, char* argv[]){
    ...
    try{
                ...
                if ((*(*it2)).getKind() == DOTWORD) {
                    ...
                    if ((*(*it2)).getLexeme() == "jr" || (*(*it2)).getLexeme() == "jalr" || (*(*it2)).getLexeme() == "lis" || (*(*it2)).getLexeme() == "mfhi" || (*(*it2)).getLexeme() == "mflo") {
                        ...
                        if (it2 != it->end()) {
                            ...
                            if ((*(*it2)).getKind() != REGISTER) {
                                cerr << "ERROR: invalid operands for sw or lw instruction" << endl;
                                error = true;
                                break;
                            }
                            ...
                            if (it2 != it->end()) {
                                ...
                                if ((*(*it2)).getKind() != COMMA) {
                                    cerr << "ERROR: operand not seperated by comma for sw or lw instruction" << endl;
                                    error = true;
                                    break;
                                }
                                ...
                                if (it2 != it->end()) {
                                    ...
                                    if ((*(*it2)).getKind() != INT && (*(*it2)).getKind() != HEXINT) {
                                        ...
                                        cerr << "ERROR: int value specified for operand i is not in range for sw or lw instruction" << endl;
                                        error = true;
                                        break;
                                    } else if ((*(*it2)).getKind() == HEXINT && ((*(*it2)).toInt() < 0 || (*(*it2)).toInt() > 65535)) {
                                        cerr << "ERROR: hex value specified for operand i is not in range for sw or lw instruction" << endl;
                                        error = true;
                                        break;
                                    }
                                    ...
                                    if (it2 != it->end()) {
                                        ...
                                        if ((*(*it2)).getKind() != LPAREN) {
                                            cerr << "ERROR: missing left bracket for the sw or lw instruction" << endl;
                                            error = true;
                                            break;
                                        }
                                        ...
                                        if (it2 != it->end()) {
                                            ...
                                            if (it2 != it->end()) {
                                                ...
                                                if ((*(*it2)).getKind() != RPAREN) {
                                                    cerr << "ERROR: missing right bracket for the sw or lw instruction" << endl;
                                                    error = true;
                                                    break;
                                                }
                                                ...
                                                if (it2 != it->end()) {
                                                    cerr << "ERROR: too many parameters for sw or lw instruction" << endl;
                                                    error = true;
                                                }
                                                
                                                break;
                                            } else {
                                                cerr << "ERROR: not enough operands for sw or lw instruction" << endl;
                                                error = true;
                                                break;
                                            }
                                        } else {
        ...
        int word2 = (16 << 24) | (0 << 16) | (0 << 8) | (2 & 0xff);
        ...
        int endFile = 8*reloc.size() + pc;
        
        // .word endFile
        putchar((endFile>>24) & 0xff);
        putchar((endFile>>16) & 0xff);
        putchar((endFile>>8) & 0xff);
        putchar(endFile & 0xff);
            ...
            for(it4 = it3->begin(); it4 != it3->end(); ++it4){
                ...
                if ((*(*it4)).getKind() == DOTWORD) {
                    ...
                    if ((*(*it4)).getLexeme() == "jr" || (*(*it4)).getLexeme() == "jalr" || (*(*it4)).getLexeme() == "lis" || (*(*it4)).getLexeme() == "mfhi" || (*(*it4)).getLexeme() == "mflo") {
                        ...
                        putchar((word>>8) & 0xff);
                        putchar(word & 0xff);
                    } else if ((*(*it4)).getLexeme() == "sw" || (*(*it4)).getLexeme() == "lw") {
                        pc += 4;
                        ...
                        if (instruction == "sw") {
                            word = (43 << 26) | (reg1 << 21) | (reg2 << 16) | (i & 0xffff);
                        } else {
                            word = (35 << 26) | (reg1 << 21) | (reg2 << 16) | (i & 0xffff);
                        }
        ...
        for (int x = 0; x < reloc.size(); x++) {
            ...
            // .word location
            putchar((reloc[x]>>24) & 0xff);
            putchar((reloc[x]>>16) & 0xff);
            putchar((reloc[x]>>8) & 0xff);
            putchar(reloc[x] & 0xff);
        }
