    ...
    try{
        ...
        for(int i = 0; i < tokLines.size() && !error; i++){
            ...
            for(int j = 0; j < tokLine.size(); ++j){
                ...
                if (word && tokLine.at(j)->getKind() != 1 && tokLine.at(j)->getKind() != 2 && tokLine.at(j)->getKind() != 0) {
                    cerr << "ERROR: no int or hex after .word" << endl;
                    error = true;
                    break;
                } else if(!word && (tokLine.at(j)->getKind() == 1 || tokLine.at(j)->getKind() == 2 || tokLine.at(j)->getKind() == 0)) {
                ...
                if (tokLine.at(j)->getKind() == 8) {
                    ...
                    command = true;
                    pc += 4;
                } else if (tokLine.at(j)->getKind() == 1 || tokLine.at(j)->getKind() == 2 || tokLine.at(j)->getKind() == 0) {
        ...
        for (it = tokLines.begin(); it != tokLines.end() && !error; ++it){
            ...
            for (it2 = it->begin(); it2!=it->end(); ++it2){
                ...
                } else if ((*(*it2)).getKind()==0) {
		     if (labels.count((*(*it2)).getLexeme()) == 0) {
                        cerr << "ERROR: uninitialized label used" << endl;
                        error = true;
                        break;
                    }
		    int num = labels[(*(*it2)).getLexeme()];
                    putchar((num>>24) & 0xff);
                    putchar((num>>16) & 0xff);
                    putchar((num>>8) & 0xff);
                    putchar(num & 0xff);
